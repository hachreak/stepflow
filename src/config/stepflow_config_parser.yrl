...
var -> interceptor_op var_def : {unwrap('$1'), '$2'}.
var -> channel_op var_def : {unwrap('$1'), '$2'}.
var -> source_op var_mdef : {unwrap('$1'), '$2'}.
var -> sink_op var_mdef : {unwrap('$1'), '$2'}.

var_def -> field eq_op field map_op eol_op : bind('$1', '$3', '$4').
...
bind(Name, Module, [Vars], Config) ->
  bind(Name, Module, Vars, Config);
bind(Name, Module, Vars, Config) ->
  to_squeeze([
    to_var(Name), reserved('='), reserved('{'), to_atom(Module),
    reserved(','), reserved('{'),
      reserved('['), Vars, reserved(']'), reserved(','), to_code(Config),
    reserved('}'), reserved('}'), reserved(dot)
  ]).
bind(Name, Module, Config) ->
  to_squeeze([
    to_var(Name), reserved('='), reserved('{'), to_atom(Module),
    reserved(','), to_code(Config), reserved('}'), reserved(dot)
  ]).
